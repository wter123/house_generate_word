...
class deal_word():
    def __init__(self):
        ...
        self._i=0
        self._s1 = ""
        self._docx_path = os.path.join(self._s1,r'模板\模板.docx')
    ...
    def _insert_word(self,excel_cell, i, doc):
        ...
        p = doc.tables[0].cell(0, 2).paragraphs[0].clear().add_run(
            self._all_to_string(excel_cell.cell(i, 3).value))  # text="aaaa"
        p.font.name = u"宋体"

        # 建筑物坐标
        c = "东经（度分秒）:" + excel_cell.cell(i, 1).value + "                  北纬（度分秒）:" + excel_cell.cell(i, 2).value
        p = doc.tables[0].cell(1, 2).paragraphs[0].clear().add_run(self._all_to_string(c))  # text="aaaa"
        p.font.name = u"宋体"

        # 普查编号
        p = doc.tables[0].cell(2, 2).paragraphs[0].clear().add_run(
            self._all_to_string(excel_cell.cell(i, 0).value))  # text="aaaa"
        p.font.name = u"宋体"
        ...
        def switch_test_item(item):
            ...
            return switcher.get(item.replace(" ", ""), "nothing")
            ...
            for ii in range(len(pictrue_set)):
                if ii == 0:
                    ...
                    paragraph.clear().add_run().add_picture(
                        os.path.join(pictrue_position, pictrue_set[ii]), width=Inches(2))
    ...
    def _word_all(self,i):
        # print(i)
        doc = docx.Document(self._docx_path)
